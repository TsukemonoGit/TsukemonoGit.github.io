...
import type { ProjectData, ProcessedProject, MajorCategory, MinorCategory, CategoryHierarchy } from './types';
import { majorCategories, minorCategories, categoryMapping } from './types';

// CSVファイルを読み込む関数
export async function loadCSVData(csvPath: string): Promise<ProjectData[]> {
  const response = await fetch(csvPath);
  const csvText = await response.text();

  const result = Papa.parse<ProjectData>(csvText, {
    header: true,
    skipEmptyLines: true,
    dynamicTyping: false
  });
...
// XLSXファイルを読み込む関数
export async function loadXLSXData(xlsxPath: string): Promise<ProjectData[]> {
...
function normalizeMajorCategory(category: string): MajorCategory {
  ...
  if (majorCategories.includes(normalized as MajorCategory)) {
    return normalized as MajorCategory;
  }
  return 'その他';
}

function normalizeMinorCategory(category: string): MinorCategory {
  const normalized = category.trim();
  if (minorCategories.includes(normalized as MinorCategory)) {
    return normalized as MinorCategory;
  }
  return '未分類';
}

// データを処理する関数
export function processProjectData(rawData: ProjectData[]): ProcessedProject[] {
...
// 小分類ごとにプロジェクトをグループ化
export function groupByMinorCategory(projects: ProcessedProject[]): Record<MinorCategory, ProcessedProject[]> {
  ...
  Object.keys(groups).forEach(minorCat => {
    groups[minorCat as MinorCategory].sort((a, b) => {
      if (a.year !== b.year) return b.year - a.year;
      return b.month - a.month;
    });
  });
